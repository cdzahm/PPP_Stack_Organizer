...
- `readFrameIntervalSec(xmlPath, nXY)` — wall-clock delta between consecutive visits to the **same** XY position, i.e. `<Sequence>` cycle `i` vs. cycle `i + nXY`, not between consecutive `<Sequence>` elements overall. This was a real bug caught during v0.2.0 GUI testing: PrairieView's `<Sequence>` elements cycle through positions round-robin (A, B, A, B, ...) for multi-position acquisitions, so a naive consecutive-cycle delta measures the position-switch time (~1/nXY of the true interval) rather than the per-position revisit interval — confirmed on both real nXY=2 sample datasets, where the naive calculation returned ~28–29s against a true 60s interval. Cycles are grouped by `(index % nXY)`; the **first** delta within a group is used rather than an average across all deltas in that group, deliberately: acquisitions can pause between cycles (stage settling, user-triggered resume), which would skew an average; the first interval is the most reliable read of the acquisition's configured cycle period. The result is **rounded to the nearest whole second** before being returned: frame interval is conceptually a round setpoint the operator chose (e.g. 60s), and raw measured deltas always carry a small amount of timestamp jitter (59.99s, 60.06s confirmed on the two real sample datasets) that isn't an intentionally-set fractional interval. A per-position-group disagreement warning was added and then removed in the same v0.2.0 bugfix pass: it fired on every real dataset tested (~1.2–1.8s of jitter between groups), making it noise rather than a useful signal. The dialog field remains user-editable, so a genuinely irregular or fractional interval can still be entered by hand. (Judgment call — revisit if real data shows the first delta is atypical, or if a fractional-second interval is ever intentional for this pipeline.)
...
- **Calibration:** XY and Z from `PrairieXmlUtils`/metadata (both unambiguously microns), time interval from `PrairieXmlUtils`/user input — carried via the `ImagePlus` `Calibration` object, round-trips through standard TIFF natively (confirmed via direct testing: reopening a saved file in Fiji shows correct pixel size, voxel depth, unit, and frame interval). `CalibrationUtils.applyCalibration()` explicitly calls `setXUnit()`/`setYUnit()`/`setZUnit()` in addition to `setUnit()`. Bytecode inspection of `ij-1.54p.jar` confirmed the exact mechanics: `getYUnit()`/`getZUnit()` fall back to the shared `unit` field whenever the `yunit`/`zunit` fields are `null`, and — separately — `FileSaver` only ever writes explicit `yunit=`/`zunit=` tags into the TIFF description when they *differ* from the X unit. Since this pipeline always sets X/Y/Z to the same "micron" string, `FileSaver` never writes `yunit=`/`zunit=` tags regardless of whether `setYUnit()`/`setZUnit()` were called — the explicit calls are a no-op for the saved bytes. They are also a no-op for correctness: `ij.plugin.filter.ImageProperties` (Fiji's Image > Properties dialog, confirmed via bytecode) reads units through `getXUnit()`/`getYUnit()`/`getZUnit()`, which already resolve to "micron" on reopen via the same null-fallback. Both a synthetic round-trip and a real reopened output file (`AB D28-034_XY01_tcells.tif`) confirm all three axes report "micron" through this getter path with or without the explicit calls. A "-" shown for Y/Z units in Properties, if it recurs, is therefore not explained by anything in this pipeline's own save/reload path — it likely comes from whichever specific tool/import path was used to view the file (e.g. opening via Bio-Formats' importer instead of Fiji's native TIFF opener, which may populate `yunit`/`zunit` as empty strings rather than leaving them `null` and so skip the fallback). The explicit `setYUnit()`/`setZUnit()` calls are kept as harmless defensive code but do not change what's written to disk.
...
- **Bio-Formats internal logging is suppressed** (`DebugTools.setRootLevel("ERROR")`, called once in a static initializer in `BioFormatsUtils`) — a real bug caught during v0.2.0 GUI testing: Bio-Formats' readers log per-plane/per-IFD status at SLF4J INFO level ("Reading IFDs", "Populating metadata", "Checking comment style", "Populating OME metadata"), which flooded the console before the first dialog even appeared, for every per-plane file the old-format datasets touch (hundreds to thousands of files). The plugin now shows its own single-line `IJ.showStatus(...)` message during metadata pre-read and file open instead, so the user sees "working," not raw internal reader chatter.
- **Persistent "it started" log entry:** `IJ.showStatus(...)` alone proved too easy to miss — it sits in the ImageJ status bar, looks idle at a glance, and disappears on mouseover. The pre-read step (which can take ~2 minutes on old-format datasets with thousands of per-plane files) now also writes `"Assessing metadata — this may take a few minutes for large acquisitions..."` to the persistent IJ log via `LogUtils.log(...)` at the start of metadata pre-read, so there's a durable, easy-to-see "it started and isn't frozen" signal alongside the status-bar message.
