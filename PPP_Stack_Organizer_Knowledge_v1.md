...
- **`IJ.showStatus(...)` alone is not a reliable "still working" signal** — it's easy to miss (status bar, disappears on mouseover). A follow-up v0.2.0 bugfix pass added a persistent `LogUtils.log(...)` entry ("Assessing metadata — this may take a few minutes for large acquisitions...") at the start of metadata pre-read, alongside the existing `IJ.showStatus` call, specifically for old-format datasets where pre-read can take ~2 minutes.
...
- **Follow-up fix (same v0.2.0 pass, second round of manual GUI testing):** the raw 59.99s/60.06s prefill and the per-position-group disagreement warning were both judged wrong. Frame interval is a round operator-chosen setpoint, not a fractional value — the raw measured deltas are timestamp jitter, not signal. `readFrameIntervalSec()` now rounds its result to the nearest whole second (`Math.round`) before returning; both real datasets now prefill exactly `60.0`. The disagreement warning was **removed entirely** — it fired on both real sample datasets on every test run (~1.2–1.8s jitter between groups, e.g. NEW: 59.99s vs 58.17s; OLD: 60.06s vs 58.84s), which is normal per-cycle timing jitter, not a real signal worth surfacing; a warning that always fires is noise. The dialog field is still user-editable, so a non-standard interval can still be entered by hand. Verified rounding applies to both the single-channel and merged/composite output write paths (both call `CalibrationUtils.applyCalibration()` with the same rounded `frameInterval` value) — confirmed `60.0` on reopen for both `_XY01.tif` (composite) and `_XY01_tcells.tif` (single-channel) on both real datasets.
...
- **Follow-up correction (same v0.2.0 pass, GUI symptom re-reported as "-" for Y/Z units in Fiji's Image > Properties, despite Show Info reporting all three axes correctly):** bytecode inspection of `ij.io.FileSaver` confirmed `yunit=`/`zunit=` tags are written to the TIFF description **only when they differ from the X unit** — since this pipeline always sets X/Y/Z to the identical string `"micron"`, `FileSaver` never writes `yunit=`/`zunit=` regardless of whether `setYUnit()`/`setZUnit()` are called. The explicit calls are a **no-op for the saved bytes**. Separately, bytecode inspection of `ij.plugin.filter.ImageProperties` (the Properties dialog) confirmed it reads units via `getXUnit()`/`getYUnit()`/`getZUnit()`, which already resolve to `"micron"` via the null-fallback with or without the explicit `setYUnit`/`setZUnit` calls — confirmed on a real reopened output file (`AB D28-034_XY01_tcells.tif`) via the same getter path. So neither this pipeline's write path nor its read-back path (via `ij.io.Opener`, the getter methods the dialog itself calls) explains a "-" in Properties. If the symptom recurs, the most likely explanation is the specific tool/import path used to view the file — e.g. opening via Bio-Formats' importer instead of Fiji's native TIFF opener, which may populate `yunit`/`zunit` as empty strings (`""`) rather than leaving them `null`, defeating the fallback. The explicit `setYUnit`/`setZUnit` calls are kept as harmless defensive code (documented as such in DESIGN.md §5.6) but do not change behavior.
